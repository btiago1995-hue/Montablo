...
export function PricingToggle({ features }: { features: string[] }) {
  ...
  return (
    <>
      ...
      {/* Card */}
      <div className="bg-white border border-border rounded-[20px] p-8 sm:p-12 max-w-sm mx-auto hover:shadow-xl hover:shadow-black/[0.04] transition-shadow duration-300 relative">
        {annual && (
        ...
        <div className="flex items-baseline justify-center gap-1 mb-1">
          <span className="font-serif text-[48px] sm:text-[52px] text-foreground">
        ...
        <ul className="text-left mb-8 space-y-0">
          {features.map((f) => (
            <li key={f} className="flex items-center gap-3 py-2.5 border-b border-border/40 last:border-b-0 text-base sm:text-sm text-foreground">
              <Check className="w-4 h-4 text-accent-dark shrink-0" />
              {f}
            </li>
          ))}
          <li className="flex items-center gap-3 py-2.5 text-base sm:text-sm text-foreground">
            <Check className="w-4 h-4 text-accent-dark shrink-0" />
            Import menu par IA
          </li>
        </ul>
        ...
        <Link
          href="/signup"
          className="group flex items-center justify-center gap-2 w-full bg-primary text-white font-medium py-4 sm:py-3.5 rounded-full hover:bg-primary-light transition-all hover:shadow-lg hover:shadow-primary/15 text-base sm:text-[15px]"
        >
